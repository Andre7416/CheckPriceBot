...
class Parse:
    ...
    def get_info(self):
        if self.url.find("https://www.farfetch.com/ru") != -1:
            ...
            title2 = self.soup.find(
                "span", attrs={"data-tstid": "cardInfo-description"}
            )
        else:
    ...
    def get_price(self):
        if self.url.find("https://www.farfetch.com/ru") != -1:
            sale_price = self.soup.find(
                "strong", attrs={"data-tstid": "priceInfo-onsale"}
            )
            not_sale_price = self.soup.find(
                "span", attrs={"data-tstid": "priceInfo-original"}
            )
        else:
            sale_price = self.soup.find(
                "div", class_="MuiTitle4-title4 currentPrice_135Ct"
            )
            not_sale_price = self.soup.find(
                "span", class_="MuiBody1-body1 oldPrice_Nd18A"
            )
        ...
        if not_sale_price:
            ...
            not_sale_price = (
                not_sale_price.replace(",", "").replace(" ", "").replace("руб", "")
            )
